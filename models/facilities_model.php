<?php
  ...
  class FacilitiesModel extends TranslatedModel {
      ...
      protected $search_fields = array("facility_translations.name", "facility_translations.description");
      ...
      public function get_all_from_last_import() {
        $model_imports = new FacilityModelImports();
        ...
        if (!empty($imports) && count($imports) > 0) {
          $last_import = $imports[0]["facility_imports"];
        }

        if ($last_import) {
          $import_id = $last_import["id"];

          $facilities = $this->get_all_with_deleted("import_id = '$import_id'");
        }

        $ids = array();

        for ($i = 0; $i < count($facilities); $i++) {
          $id = $facilities[$i]["facilities"]["id"];
          $facilities[$i]["status"] = $this->compare_with_previous($id, $facilities[$i]);

          array_push($ids, $id);
        }

        $ids = implode(",", $ids);
        $other_facilities = $this->get_all("id NOT IN ($ids)");
      ...
      public function compare_with_previous($id, $item = NULL) {
        ...
        $identifier = $item["facilities"]["identifier"];
      ...
      public function import($file) {
        ...
        $import_model = new FacilityModelImports();
        $import = $import_model->create();
        $import_id = $import["main"]["message"];

        $all_facilities = $this->get_all();
        $all_facilities_objects = array();
        $all_facilities_ids = array();

        for ($i = 0; $i < count($all_facilities); $i++) {
          $facility = $all_facilities[$i];

          if (isset($facility["facility"])) {
            $identifier = $facility["facility"]["identifier"];
            $all_facilities_ids[$identifier] = FALSE;
            $all_facilities_objects[$identifier] = $facility;
          }
        }
        ...
        foreach ($features as $feature) {
          ...
          $all_facilities_ids[$identifier] = TRUE;
          ...
          if (count($existing) > 0) {
            foreach ($existing as $element) {
              if (!isset($element["facilities"])) {
                continue;
              }

              // We need to insert existing labels into new object
              if ($element["facility_labels"]) {
                foreach ($element["facility_labels"] as $label) {
          ...
          if (count($labels) > 0) {
            $new_facility["facility_labels"] = $labels;
          }

          $inserted = $this->create($new_facility);
        }
...
////////////////////////////////////////////////////////////////////////////////
  ...
  class FacilityModelImports extends \Singular\Model {
    protected $table = "facility_imports";
